...
plus the chat-turn domain transactions.
...
The two turn transactions (``create_user_and_assistant_atomic``,
``update_turn_terminal``) live here rather than in ``bibilab.db`` because
they encode the turn-creation / turn-terminal state machine over the
messages + conversations tables — domain rules, not pure SQL. The
corresponding CRUD stays in ``bibilab.db`` and is imported on demand.
"""
...
from bibilab.db.messages import IN_FLIGHT_ASST_STATUS, IN_FLIGHT_USER_STATUS
...
# Re-exported so callers can still import the public surface from this module
# alongside the streaming primitives.
__all__ = [
