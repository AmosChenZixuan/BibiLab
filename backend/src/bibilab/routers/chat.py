...
def build_grounding_prompt(response_language: str) -> str:
    ...
    return (
        ...
        "0a. When the user names a specific source by title, episode number, "
        "chef name, or other identifier (e.g. '第八集讲了什么', '第3道菜', 'the React video'), "
        "include source_filter={'title_contains': '<identifier>'} in the retrieve call so "
        "retrieval is scoped to that subset. "
        "Use expected_hits='many' when the user wants a comprehensive summary of one source "
