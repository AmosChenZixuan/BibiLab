...
def test_constants_match_poc_validated_values():
    # Tuned for the live ~20-min zh video corpus. See the module docstring
    # in `section.py` for the data-driven tuning rationale.
...
def test_derive_sections_single_segment_yields_one_section():
    ...
    secs = derive_sections(segs)
    assert len(secs) == 1
    assert secs[0].seg_start == 0
    assert secs[0].seg_end == 0
    assert secs[0].token_count == count_tokens("hello world.")
...
def test_derive_sections_picks_longest_pause_in_zone():
    ...
    secs = derive_sections(segs, target_tokens=200)
...
def test_derive_sections_trailing_remainder_is_emitted():
    ...
    secs = derive_sections(segs, target_tokens=10_000)
