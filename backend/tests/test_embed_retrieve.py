...
def _build_collection(source_id: str = "v1", n_chunks: int = 6):
    """Build an in-memory Chroma collection with `n_chunks` sequential chunks."""
    client = chromadb.EphemeralClient()
    collection = client.create_collection(f"test_{source_id}_{uuid.uuid4().hex[:8]}")
    collection.add(
        ids=[f"{source_id}_{i}" for i in range(n_chunks)],
...
def patch_chroma(monkeypatch):
    """Install a Chroma collection + identity source_id resolution."""
...
def patch_rerank(monkeypatch):
    ...
    async def _fake_rerank(query, chunks, top_k):
        for c in chunks:
            cid = f"{c.source_id}_{c.sequence_index}" if c.sequence_index is not None else c.source_id
...
class TestParseSeqIndex:
    ...
    def test_source_id_with_underscore(self):
        # rsplit on _ takes the trailing integer
        assert _parse_seq_index("my_source_id_42") == 42
...
class TestChunksFromChromaGet:
    ...
    def test_flat_shape(self):
        rows = {
            ...
            "documents": ["d0", "d2"],
            "metadatas": [
                {"source_id": "v1", "video_title": "T", "timestamp_start": 0.0, "timestamp_end": 10.0},
                {"source_id": "v1", "video_title": "T", "timestamp_start": 20.0, "timestamp_end": 30.0},
            ],
        }
...
async def test_cross_source_neighbors_in_both(patch_chroma, patch_rerank, monkeypatch):
    ...
    by_source = {}
    for c in result.chunks:
        by_source.setdefault(c.source_id, []).append(c.sequence_index)
