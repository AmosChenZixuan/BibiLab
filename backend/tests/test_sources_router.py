...
async def test_get_source_sections_null_row_fields_are_defaulted(client: httpx.AsyncClient, tmp_bibilab_home: Path):
    """Legacy rows (pre-section-digests path) have NULL summary/keywords/timestamps.

    The router must project these to safe defaults instead of 500ing or
    returning nulls — fail-graceful on the read path.
    """
    from bibilab.db import bootstrap_db, create_list, get_db, write_source_with_segments
    from bibilab.pipeline.section import Section
    from bibilab.pipeline.transcribe import WhisperSegment

    await bootstrap_db()
    await create_list("list-nulls", "Nulls", "2025-01-01T00:00:00Z")
    sid = await SourceFactory.build("list-nulls", video_id="BVnulls", title="Nulls", summary="x", keywords=[])
    segments = [WhisperSegment(start=float(i), end=float(i + 1), text=f"s{i}.", speaker=None) for i in range(10)]
    sections = [Section(seg_start=0, seg_end=9, token_count=100, timestamp_start=0.0, timestamp_end=10.0)]
    # section_digests=None → NULL summary/keywords in the DB.
    await write_source_with_segments(
        segments=segments,
        sections=sections,
        source_id=sid,
        ...
        keywords=[],
        cover_url=None,
        source_url="https://www.bilibili.com/video/BVnulls",
        duration_seconds=10,
        uploader="U",
        language="zh",
        whisper_model="x",
    ...
    # Manually NULL out the timestamps to exercise the NULL-projection path.
    async with get_db() as db:
        await db.execute(
            "UPDATE sections SET timestamp_start=NULL, timestamp_end=NULL WHERE source_id=?",
            (sid,),
        )
        await db.commit()
