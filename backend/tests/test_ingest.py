...
@pytest.mark.asyncio
async def test_ingest_multiple_videos(client: httpx.AsyncClient):
...
@pytest.mark.asyncio
async def test_ingest_resolve_not_called(client: httpx.AsyncClient):
    """POST /ingest/url must NOT call BilibiliAdapter.resolve()."""
    from bibilab.adapters.bilibili import BilibiliAdapter

    list_id = (await client.post("/lists", json={"name": "Test"})).json()["id"]

    with patch.object(BilibiliAdapter, "resolve") as mock_resolve:
        resp = await client.post(
            "/ingest/url",
            json={"list_id": list_id, "videos": [_video_payload()]},
        )
...
@pytest.mark.asyncio
async def test_ingest_queued_order_matches_input(client: httpx.AsyncClient):
