...
- **RAG metadata**: The LLM dispatches a `retrieve` tool call mid-stream; results arrive as an SSE `tool_result` event with `name: "retrieve"`. Parsed in `useSSEStream` and surfaced via `RetrievalLedger`. See `RagMetadata` type in `@/lib/chat-utils`. On history reload, `useConversationHistory` reads `metadata.rag` from the stored message. `RetrievalCall`: `scoped_pool_size` (the **full** pool — exclude/whitelist scoping removed), `facet_scope?` (the live pre-retrieval scoping signal), `gate_margin`. `scope_choice` / `excluded_count` still exist in the type but the backend no longer populates them — treat as absent. `context?` is **optional** — the streaming `tool_result` payload omits it (reconstructed only at persist), so `callVariant` treats absent (not `[]`) `context` as default, not `empty`. A final `rag` SSE event (just before the terminal) carries the authoritative persisted-shape calls and replaces the in-memory ledger so expand works post-stream without a refresh. While `msg.isStreaming`, `RetrievalLedger` rows render collapsed and non-expandable (`streaming` prop) since `context[]` is incomplete until that `rag` event. `RetrievalChunk` includes `citation_index`, `source_id`, `source_title`.
