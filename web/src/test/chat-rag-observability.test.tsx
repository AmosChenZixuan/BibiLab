...
describe("RAG observability via SSE tool_result", () => {
  ...
  test("two parallel retrieve calls produce two pending rows replaced independently", async () => {
    vi.spyOn(window, "fetch").mockImplementation(() =>
      Promise.resolve(
        makeSseStream([
          'data: {"type":"tool_call_start","id":"tc1","name":"retrieve","arguments":{"query":"A","mode":"survey"}}\n\n',
          'data: {"type":"tool_call_start","id":"tc2","name":"retrieve","arguments":{"query":"B","mode":"narrow"}}\n\n',
          'data: {"type":"tool_result","id":"tc1","name":"retrieve","result":{"query":"A","mode":"survey","candidates_evaluated":10,"sources_with_hits":2,"sources_total":4,"source_coverage":[{"source_id":"s1","video_id":"v1","title":"Video A"},{"source_id":"s2","video_id":"v2","title":"Video B"}],"context":[{"chunk_id":"c1","citation_index":1,"source_id":"s1","source_title":"Video A","timestamp_start":0,"timestamp_end":60,"rerank_score":3.5,"preview":"preview A"}],"dropped_by_gate":0,"reranked":true,"scoped_pool_size":4,"gate_margin":null}}\n\n',
          'data: {"type":"tool_result","id":"tc2","name":"retrieve","result":{"query":"B","mode":"narrow","candidates_evaluated":20,"sources_with_hits":1,"sources_total":3,"source_coverage":[{"source_id":"s3","video_id":"v3","title":"Video C"}],"context":[{"chunk_id":"c2","citation_index":2,"source_id":"s3","source_title":"Video C","timestamp_start":10,"timestamp_end":90,"rerank_score":2.8,"preview":"preview B"}],"dropped_by_gate":0,"reranked":true,"scoped_pool_size":2,"gate_margin":0.1}}\n\n',
          'data: {"type":"delta","content":"Answer"}\n\n',
          'data: {"type":"done"}\n\n',
        ]),
      ),
  ...
  test("empty context with dropped_by_gate renders empty variant", async () => {
    vi.spyOn(window, "fetch").mockImplementation(() =>
      Promise.resolve(
        makeSseStream([
          'data: {"type":"tool_result","name":"retrieve","result":{"query":"narrow query","mode":"narrow","candidates_evaluated":5,"sources_with_hits":0,"sources_total":3,"source_coverage":[],"context":[],"dropped_by_gate":3,"reranked":false,"scoped_pool_size":1,"gate_margin":null}}\n\n',
