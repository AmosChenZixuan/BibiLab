...
| Retrieval | Deterministic facet scoping (fail-open) → hybrid BM25+vector RRF → cross-encoder rerank → **gateless top-k by rerank order** → section-keyed fence render with speaker-turn reconstruction | Two-tool surface: `find_passages` (recall-biased locator) + `read_section` (ranking-immune verbatim read); rerank is ordering, not authority; LLM *extracts* facets, backend *matches* them deterministically. Full pipeline: `docs/chat_architecture.md` |
| Tool-calling chat | `stream_with_tools` wraps `stream_llm` in a bounded loop (max 3 iterations); loopback tools; exhausted iterations keep tools advertised but unexecuted + synthesis directive | Eliminates the pre-stream classifier — the LLM decides which retrieval tool (if any) to use, with subject decomposition for parallel calls. Details: `docs/chat_architecture.md` |
| Chat streaming | SSE via `StreamingResponse`; 9 event types, `meta` first (cancel-by-id); preamble streamed immediately; `[N]` citations as `citation` events; machine-readable error codes for i18n | Final `rag` event carries the authoritative ledger before the terminal. Details: `docs/chat_architecture.md` |
