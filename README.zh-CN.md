...
| 开源                                                 |    ✓    |     ✗      |
| 视频来源:B 站 · YouTube · TikTok                    |    ✓    | 仅 YouTube |
| 带说话人标注的转写                                   |    ✓    |     —      |
| 回链到来源的行内引用                                 |    ✓    |     ✓      |
...
3. **入库视频** —— 在列表详情页粘贴 B 站 / YouTube / TikTok 链接。B 站支持单视频、
   多分P、合集与私人收藏夹(需 cookie 登录);YouTube 支持单视频与公开播放列表
   (免登录);TikTok 支持单视频(含分享短链)与公开合集(免登录,尽力支持——
   上游提取器周期性失效,升级 yt-dlp 通常可修复)。流水线在后台运行:
