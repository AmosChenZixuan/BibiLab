...
| `backend/src/bibilab/pipeline/chat_loop.py` | `SSE_EVENT_CITATION`, `stream_with_tools` registry+parser integration, `build_grounding_prompt` (section-grained, with #396 coverage-exemption) |
| `backend/src/bibilab/routers/chat.py` | `run_chat_turn` content_blocks accumulation + persistence, SSE serialization |
