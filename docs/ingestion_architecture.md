...
3. **transcribe** → sherpa-onnx: Silero VAD + SenseVoice/Whisper recognition + CAM++ speaker-embedding clustering → raw VAD segments with timestamps + speaker labels.
...
6. **chunk** → greedily merge consecutive **sentence** segments within each section up to `DOC_TOKEN_BUDGET` (444 XLM-R tokens — the unit the embedder and reranker actually tokenize with), split at trustworthy sentence boundaries. The ceiling is enforced, not a target: the only chunk that may exceed it is a single segment already over it, emitted verbatim. Records `seg_start`/`seg_end` per chunk (source-global indices). Chunks physically nest in sections — a chunk's `[seg_start, seg_end]` is fully contained in exactly one section's range.
